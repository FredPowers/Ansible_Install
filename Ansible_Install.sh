...
then
		...
		useradd $login --create-home -s /bin/bash && echo -e ""$vert"Utilisateur $login créé$normal"
		echo
		sleep 2
		usermod -aG sudo $login && echo -e ""$vert"L'tilisateur $login a été intégré au groupe sudo$normal"
		sleep 2
		passwd $login
		echo
...
do
	...
	1)
		...
		liste_apt=( "gcc" "openssh-client" "openssh-server" "python3" "python3-pip" "python-dev" "libkrb5-dev" "ansible"  )
		...
		liste_apt1=( "kerberos" "pykerberos" "requests_kerberos" "pywinrm" )

		for pkg1 in ${liste_apt1[@]}
			do
				pip install $pkg1
